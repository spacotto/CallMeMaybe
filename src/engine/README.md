...
This module contains the core generation loop and the mathematical state machine responsible for forcing the Large Language Model (LLM) to output strictly valid JSON schemas.

## Architecture Overview

Instead of relying on probabilistic text generation, this engine intercepts the neural network's raw `logits` output at every step of the autoregressive loop. By applying dynamic, vectorised boolean masks via NumPy, the engine mathematically eliminates the probability of any token that would violate the target JSON structure.

## Performance Optimisations (Caching & Batching)

To achieve high throughput on resource-constrained hardware, the engine implements exhaustive caching across multiple layers of the architecture to eliminate redundant computations.

### 1. Vectorised State Caching (Precomputed Masks)
Instead of iterating through the 150,000+ token vocabulary on every generation step to check character validity, the engine caches the entire validation state during initialization.
...
The engine intercepts expensive string manipulations and tokenizer decoding operations.
* **Implementation:** The `clean_vocab` list caches the results of byte-pair string replacements (`replace("Ġ", " ")`). The `self.fast_decode` method utilizes Python's `@lru_cache`.
