"""
Phase 2 engine module: Context-Aware Argument Extraction.

This module is responsible for the complex generation of function
arguments. It tracks the structural state of the generated JSON
string in real-time and dynamically masks invalid tokens to ensure
absolute adherence to the provided JSON schema.
"""
...
class JSONParserState:
    ...
    Rather than continuously parsing the entire generated string
    with a heavy JSON library or Regex, this state machine updates
    its current context character-by-character. It tracks depth
    and expected syntax to inform the masking engine exactly what
    structural characters (like colons or commas) are legally allowed.

    Attributes:
        state (str): The current expected syntax (e.g., 'EXPECT_KEY').
        stack (List[Dict[str, Any]]): Tracks nested object contexts.
        escape (bool): Flags if the current character is escaped.
        current_key (str): The buffer for the key currently being generated.
        pending_key (str): The last completed key waiting for a value.
        depth (int): Tracks bracket nesting to determine completion.
    """

    def __init__(self) -> None:
        """Initializes the parser state to expect top-level parameters."""
    ...
    def update(self, new_chars: str) -> None:
        """
        Updates the internal state machine based on newly generated text.

        Args:
            new_chars (str): The latest characters generated by the LLM.
        """
...
class SchemaExtractor:
    ...
    This class orchestrates the generation loop for the function arguments.
    It dynamically enforces the JSON schema shape by constantly updating
    boolean masking arrays based on the current parser state.

    Attributes:
        sdk (Small_LLM_Model): The initialized LLM wrapper.
        tokenizer (Tokenizer): Custom token handler.
        formatter (Formatter): Prompt template assembler.
        vocab_size (int): Upper bound of token IDs.
        clean_vocab (List[str]): Lookup list for decoded tokens.
    """

    def __init__(self, classifier_instance: FunctionClassifier) -> None:
        """
        Initializes the extractor and pre-computes vocabulary masks.

        Args:
            classifier_instance (FunctionClassifier): The Phase 1 engine
                instance to share model, tokenizer, and formatter states.
        """
        ...
        # Instead of evaluating string rules for 150,000+ tokens during every
        # generation step, we pre-compute foundational boolean masks once
        # upon initialization. This drops generation time significantly.
        # ------------------------------------------------------------------
        ...
        # ------------------------------------------------------------------
        # [CACHING OPTIMIZATION]: Dynamic Caches
        # key_mask_cache: Memoizes calculated bitwise masks for specific
        # JSON keys to prevent recalculating masks for recurring keys.
    ...
    def _build_schema_maps(
        ...
        """
        Recursively maps a parameter schema into flat lookup dictionaries.

        Args:
            properties (Dict[str, Any]): The schema properties block.

        Returns:
            Tuple[Dict[str, List[str]], Dict[str, str]]: A tuple containing
            the key-to-subkeys map, and the key-to-type map.
        """
    ...
    def extract_batch(
        ...
        """
        Executes the batched constrained generation loop for arguments.

        Args:
            prompts (List[str]): Batch of raw natural language requests.
            function_names (List[str]): The targeted function names.
            functions (List[Dict[str, Any]]): The available schemas.
            max_new_tokens_list (List[int]): Individual token limits.
            verbose (bool): If True, triggers the visualizer callbacks.

        Returns:
            List[str]: The raw, partially generated JSON strings for the batch.
        """
        input_sequences = []
        generated_sequences = []
        current_prefixes = []
        ...
        for step in range(absolute_max_steps):
            ...
            # ------------------------------------------------------------------
            # [BATCHING MECHANISM]: Matrix Parallelization
            # Collect the raw logits for all unfinished prompts in the batch.
            # By stacking them into a 2D numpy array, we allow the CPU/GPU to
            # apply our mathematical masks to all prompts simultaneously.
            # ------------------------------------------------------------------
            batch_logits = []
            for i in active_idx:
    ...
    def _get_mask(
        ...
        Args:
            parser_state (JSONParserState): The current structural state.
            key_map (Dict[str, List[str]]): The allowed schema hierarchy.
            type_map (Dict[str, str]): The expected parameter data types.

        Returns:
            np.ndarray: A boolean array where True allows token generation.
        """
        ...
        if not in_string:
            ...
            if state == "IN_KEY":
                ...
                # --------------------------------------------------------------
                # [CACHING OPTIMIZATION]: Key Mask Cache
                # Masking keys requires evaluating string prefixes against all
                # possible vocabulary tokens. Memoizing this result prevents
                # redundant loop executions if the LLM generates the same key
                # string incrementally across multiple steps.
                # --------------------------------------------------------------
